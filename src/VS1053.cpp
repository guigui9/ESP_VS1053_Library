...
#define printDetails(a)  ((void*)0)
#else
#define dbgprint  SerialPrint
#endif
...
bool VS1053::testComm (const char *header) {
  ...
  // If DREQ is low, there is problably no VS1053 connected.
  // Pull the line HIGH in order to prevent an endless loop waiting for this signal.
  // The rest of the software will still work, but readbacks from VS1053 will fail.
  ...
  for (uint32_t i = 0; (i < 0xFFFF) && (cnt < 20); i += delta) {
...
void VS1053::playChunk (const uint8_t *data, size_t len) {
  sdi_send_buffer ((uint8_t *)data, len);
}
